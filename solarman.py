...
def get_token(url, appid, secret, username, password, orgId=None):
    """
    Get a token from the Solarman API
    :return: access_token or None
    """
    print(f"{time_stamp()}: 🕵️  Getting token from: {url}")

    try:
        # hash password (ak chceš, môžeš aj použiť plain heslo priamo z configu)
        passhash = hashlib.sha256(password.encode()).hexdigest()

        # vytvorenie HTTPS spojenia
        ...
        path = f"/account/v1.0/token?appId={appid}&language=en"  # správna cesta

        # odoslanie POST requestu
        conn.request("POST", path, payload, headers)
        res = conn.getresponse()

        # dekódovanie bytes -> str a parsing JSON
        raw = res.read().decode()
        print(f"RAW RESPONSE: {raw}")
        data = json.loads(raw)

        # debug: vypíšeme si celý response
        print(f"{time_stamp()}: 🔥🔥🔥 Token received successfully, raw response: {data}")

        # bezpečné získanie tokenu
        access_token = data.get("access_token")
        if access_token:
            return access_token
        else:
            print(f"{time_stamp()}: ❌ access_token not found in response")
            return None

    except Exception as error:  # pylint: disable=broad-except
        print(f"{time_stamp()}: 😡 Unable to 555 fetch token: {str(error)}")
        return None
